...
>
> 变更记录:
> - v0.3.0: 上下文预算改为按模型比例分配；新增工具索引与按需加载机制；新增 Skills 能力模块
...
- [3. 功能模块规格](#3-功能模块规格)
  ...
  - [3.10 工具体系](#310-工具体系)
  - [3.11 Skills 能力](#311-skills-能力)
  - [3.12 终端 AI 增强](#312-终端-ai-增强)
  - [3.13 设置与配置](#313-设置与配置)
- [4. 性能专项设计](#4-性能专项设计)
...
- 没有上下文裁剪和自动摘要，历史原样传入
- 工具结果不压缩
- 工具定义全量注入（50+ 工具时可能消耗 10K+ Token）
- 上下文预算是硬编码值，不随模型的上下文窗口大小自适应

#### 3.7.1 功能需求

| 编号 | 需求 | 说明 |
|------|------|------|
| FR-CTX-01 | 按比例预算分配 | 上下文预算根据所选模型的 `maxInputTokens` **按比例** 动态计算，而非硬编码绝对值 |
...
| FR-CTX-09 | 工具定义动态加载 | 核心工具常驻上下文，非核心工具通过工具索引按需加载（见 3.10.5） |
| FR-CTX-10 | Skills 指令管理 | Skills 内容纳入上下文预算管理，按相关度排序注入（见 3.13） |

#### 3.7.2 按比例预算分配模型

上下文预算以 **百分比** 定义，根据模型的 `maxInputTokens` 自动换算为绝对值。
以下百分比基于 `effectiveInputBudget = maxInputTokens - maxOutputTokens`（即真正可用于 prompt 的部分）。

| 区域 | 内容 | 占比 | 8K 模型 | 32K 模型 | 128K 模型 | 200K 模型 | 可压缩 |
|------|------|------|---------|----------|-----------|-----------|--------|
| 固定区 | 系统提示词 | 3% | 240 | 960 | 3.8K | 6K | 否 |
| 固定区 | 核心工具定义（常驻） | 3% | 240 | 960 | 3.8K | 6K | 否 |
| 固定区 | 项目上下文摘要 | 2% | 160 | 640 | 2.5K | 4K | 否 |
| 固定区 | Skills 指令 | 2% | 160 | 640 | 2.5K | 4K | 按相关度裁剪 |
| 动态区 P0 | 用户消息 + 附件 + 检索结果 | 20% | 1.6K | 6.4K | 25K | 40K | 按相关度裁剪 |
| 动态区 P1 | 活跃上下文（工具结果、子 Agent 摘要） | 30% | 2.4K | 9.6K | 38K | 60K | 可压缩 |
| 动态区 P2 | 历史对话 | 20% | 1.6K | 6.4K | 25K | 40K | 旧的被摘要化 |
| 动态区 P3 | 补充上下文（README、Git Diff） | 5% | 400 | 1.6K | 6.4K | 10K | 可丢弃 |
| 弹性区 | 溢出缓冲 | 15% | 1.2K | 4.8K | 19K | 30K | 动态区溢出时使用 |

**说明**：
- 百分比之和 = 100%，弹性区为各动态区溢出时的缓冲空间
- 小上下文模型（如 8K）下，固定区占比可能不足，此时自动触发：减少核心工具数量、截断 Skills、精简系统提示
- `maxOutputTokens` 由模型元数据提供，不占上述预算（由 Provider 在请求时自动处理）

#### 3.7.3 小上下文模型自适应

当 `effectiveInputBudget` < 16K 时，触发以下自适应策略：

| 预算范围 | 自适应措施 |
|---------|-----------|
| < 16K | 核心工具定义压缩为仅 name + 单句描述（去掉 inputSchema 详情） |
| < 16K | Skills 最多注入 2 个最相关的 |
| < 16K | 历史对话只保留最近 3 轮 |
| < 8K | 禁用工具索引动态加载，仅保留 3 个核心工具 |
| < 8K | 历史对话只保留最近 1 轮 |
| < 8K | 项目摘要压缩为单句 |
...
| L4 | 代码上下文骨架化（仅签名）；非核心工具定义移除 | 使用率 > 90% |
| L5 | 仅保留当前轮次；仅保留核心工具 | 使用率 > 95% |
...
**当前 VS Code 的不足**：工具定义全量注入 LLM 上下文，没有工具索引或按需加载机制。当工具数量多（50+）时，工具定义本身可能消耗 10K-20K Token，严重挤占有效上下文空间。
...
| **toolSearch** | **工具索引搜索** | **从工具索引中检索最匹配的工具，按需加载到上下文** |
...
| FR-TL-07 | 工具索引与按需加载 | 非核心工具不直接注入上下文，通过工具索引按需检索加载 |
| FR-TL-08 | 核心工具常驻 | 最常用的核心工具始终在上下文中，无需检索 |

#### 3.10.5 工具索引与按需加载机制

**问题**：MCP 生态 + 内置工具 + 扩展工具，注册的工具总数可能达到 50-100+。每个工具定义（name + description + inputSchema JSON）平均消耗 200-500 Token。全部注入上下文可能消耗 10K-50K Token。

**解决方案**：将工具分为两层——核心工具（常驻上下文）和索引工具（按需加载）。

**核心工具（始终在上下文中）**：

这些是 Agent 几乎每次都会用到的高频工具，约 8-12 个，消耗 ~2K-3K Token：

| 工具 | 原因 |
|------|------|
| editFile | 每次代码修改都需要 |
| readFile | 每次分析都需要 |
| search (ripgrep) | 代码搜索是基础能力 |
| listDirectory | 项目浏览基础 |
| terminal | 运行命令 |
| runSubagent | 委派子任务 |
| codebaseSearch | 语义搜索 |
| toolSearch | 从索引中查找更多工具 |

**索引工具（按需加载）**：

所有其他工具（MCP 工具、扩展工具、低频内置工具），通过工具索引管理：
...
│            │                                                 │
│            ▼                                                 │
│     Agent 选择需要的工具 → 动态加载到当前会话上下文             │
│                                                              │
│  ② 系统智能推荐:                                              │
│     分析用户消息/任务描述的意图                                │
│     → 预测可能需要的工具类别                                  │
│     → 自动将相关工具加载到上下文                              │
│     (例如用户提到 "部署" → 自动加载 Docker 和 Git 工具)       │
│                                                              │
└──────────────────────────────────────────────────────────────┘
```
...
```
toolSearch:
  description: "搜索可用工具。当你需要执行一个当前工具集无法完成的操作时使用。"
  inputSchema:
    query: string       # 描述需要什么能力，如 "操作 Docker 容器"
    category?: string   # 可选类别过滤: filesystem, git, web, database, devops, docs
  output:
    tools: Array<{name, description, inputSchema}>  # 匹配的工具列表
```
...
| 方法 | 适用场景 | 说明 |
|------|---------|------|
| 关键词匹配 | 默认方式 | 基于工具 name + description 的 TF-IDF 匹配 |
| 能力标签 | 有分类的工具 | 为每个工具打标签（filesystem / git / web / database / devops） |
| 语义匹配 | 高级模式 | 对工具描述生成嵌入向量，语义相似度匹配 |

**Token 预算节省效果**：

| 场景 | 无索引（全量注入） | 有索引（核心+按需） | 节省 |
|------|-------------------|-------------------|------|
| 30 个工具 | ~10K Token | ~3K + 按需 ~1K | **60%** |
| 50 个工具 | ~18K Token | ~3K + 按需 ~1K | **78%** |
| 100 个工具 | ~35K Token | ~3K + 按需 ~2K | **86%** |

---

### 3.11 Skills 能力

**目标**：让 Agent 具备可扩展的领域知识和操作规范能力。

**复用基础**：VS Code 已有 Skills 系统（`SKILL.md` 文件 + `<skills>` 指令注入机制）。

#### 3.11.1 Skills 与 Tools 的区别

| 维度 | Tools | Skills |
|------|-------|--------|
| 本质 | 可调用的函数 | 领域知识和操作指南 |
| 形式 | JSON Schema 定义 + 代码实现 | Markdown 文档 |
| 调用方式 | LLM 通过 tool_call 调用 | LLM 读取后内化为行为准则 |
| 示例 | `editFile(path, content)` | "编写 React 组件时遵循这些模式..." |
| 上下文消耗 | 工具定义占 Token | Skill 内容占 Token |

#### 3.11.2 Skill 类型

| 类型 | 说明 | 示例 |
|------|------|------|
| 编码规范 Skill | 团队编码风格和约定 | "TypeScript 代码必须使用 strict mode，错误处理使用 Result 模式" |
| 架构知识 Skill | 项目架构和设计决策 | "本项目使用六边形架构，所有 IO 必须通过端口/适配器" |
| 领域知识 Skill | 业务领域相关知识 | "支付模块的合规要求：PCI DSS 相关规则..." |
| 操作规范 Skill | 特定操作的步骤指南 | "添加新 API 端点的标准流程：1.定义 Schema 2.实现 Handler..." |
| 工具使用 Skill | 特定工具的使用指南 | "使用数据库迁移工具时的最佳实践..." |
| 测试规范 Skill | 测试编写的标准 | "单元测试覆盖率要求、Mock 策略、测试命名规范" |

#### 3.11.3 功能需求

| 编号 | 需求 | 说明 |
|------|------|------|
| FR-SK-01 | SKILL.md 发现 | 自动发现项目中 `.github/skills/`、`.agents/skills/` 等目录的 SKILL.md |
| FR-SK-02 | 个人 Skills | 支持用户级 Skills（`~/.copilot/skills/`、`~/.agents/skills/`） |
| FR-SK-03 | 按相关度注入 | 不是所有 Skills 都注入上下文，而是根据当前任务相关度排序选择 |
| FR-SK-04 | Skill 索引 | 为所有 Skills 建立索引，支持语义匹配（类似工具索引） |
| FR-SK-05 | 手动触发 | 用户可通过 `/skillname` 手动激活特定 Skill |
| FR-SK-06 | 禁用模型自动加载 | 支持 `disable-model-invocation: true` 标记，该 Skill 仅手动触发 |
| FR-SK-07 | 隐藏 Skill | 支持 `user-invocable: false` 标记，该 Skill 对用户不可见但 Agent 可自动使用 |
| FR-SK-08 | Skill 创建向导 | 提供创建新 Skill 的交互式向导 |
| FR-SK-09 | 上下文预算感知 | Skills 内容纳入上下文预算管理（占固定区 2%），超出时按相关度裁剪 |
| FR-SK-10 | Skill + Tool 联动 | Skill 中可声明关联的工具，激活 Skill 时自动加载对应工具 |

#### 3.11.4 Skill 相关度匹配

Skills 不能全部注入上下文（尤其是项目有很多 Skills 时），需要按需选择：

```
用户消息: "帮我添加一个新的 REST API 端点"
       │
       ▼
┌──────────────────────────┐
│  Skill 相关度匹配引擎     │
...
| FR-ST-08 | Skills 管理 | 查看/创建/启停 Skills |
| FR-ST-09 | 工具管理 | 查看核心工具列表、配置工具索引 |
...
aiStudio.tools.coreTools             # 核心工具列表 (始终在上下文中)
aiStudio.tools.indexEnabled          # 工具索引启停 (默认 true)
aiStudio.tools.autoRecommend         # 根据任务自动推荐工具 (默认 true)

aiStudio.skills.enabled              # Skills 启停 (默认 true)
aiStudio.skills.locations            # Skills 搜索目录
aiStudio.skills.maxInContext         # 单次最多注入的 Skill 数 (默认 5)
...
| Skills 系统 | 复用 + 增强 | VS Code `SKILL.md` + promptSyntax |
| **AI Provider 抽象层** | **自研** | — |
| **上下文管理器（含按比例预算）** | **自研** | — |
| **工具索引与按需加载** | **自研** | — |
| **子 Agent 增强** | **自研** | — |
...
| 任务 | 说明 | 优先级 |
|------|------|--------|
| 智能代码补全 | 注册 InlineCompletionsProvider + FIM + 缓存/预取 | P0 |
| Chat 面板 | 内置 Chat Participant + 上下文引用 | P0 |
| Inline Chat 适配 | 对接自有 AI Provider | P0 |
| 上下文管理器 | 按比例预算 + 压缩策略 + Token 缓存 | P0 |
| 工具索引 | 核心工具常驻 + 非核心工具索引与按需加载 | P0 |
...
| Skills 集成 | Skills 发现 + 相关度匹配 + 按需注入 | P0 |
...
### Phase 4: 打磨与发布（第 15-20 周）

| 任务 | 说明 | 优先级 |
|------|------|--------|
| 终端 AI 增强 | 自然语言转命令 + 错误分析 | P2 |
| 设置 UI | AI 配置可视化界面（含 Skills 和工具管理） | P1 |
| MCP 管理 UI | MCP Server 可视化管理 | P2 |
| Skill 创建向导 | 交互式 Skill 创建 | P2 |
| 性能优化 | 按性能红线逐项验证和优化 | P0 |
...
Week  3-6  ████████ Phase 1: 补全 + Chat + Inline Chat + 上下文管理 + 工具索引
Week  7-10 ████████ Phase 2: Agent + 子 Agent + Skills + Diff 审查
Week 11-14 ████████ Phase 3: 代码知识引擎 (MCP → sidecar)
Week 15-20 ████████████ Phase 4: 终端增强 + 设置UI + 性能优化 + 测试 + 打包
```
...
| Skill | 以 Markdown 形式定义的领域知识和操作规范，Agent 读取后内化为行为准则 |
| Tool Index | 工具索引，对非核心工具建立关键词/语义索引，Agent 按需检索加载 |
| Contribution | VS Code 的功能注册模式 |
| DI | Dependency Injection，依赖注入 |
| TTFT | Time To First Token，从发送请求到第一个 Token 返回的延迟 |
| effectiveInputBudget | 模型可用输入预算 = maxInputTokens - maxOutputTokens |
